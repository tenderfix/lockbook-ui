...
interface ResendAccountActivationProps {
  onSubmit: (values: FormValues) => void;
}

const ResendAccountActivationStep1: FunctionComponent<ResendAccountActivationProps> = (
  props: ResendAccountActivationProps
) => {
...
const ForgotPasswordStep1Form: FunctionComponent<FormikProps<FormValues>> = (
  ...
  return (
    <Form noValidate={true} style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <S.HeadlineWrapper>
        <span>{t('new_account_activation.resend_account_activation')}</span>
      </S.HeadlineWrapper>
      <S.ContentWrapper>
        <S.TextWrapper>{t('new_account_activation.info')} </S.TextWrapper>
        <FormInput
        ...
        <Popup
          ...
          toggleNode={
            <Button as={'a'} color={'black'} variant={'text'}>
              {t('new_account_activation.forgot_username.title')}
            </Button>
          }
          popupContent={
            <S.LostAccountPopup>
              <strong>{t('new_account_activation.forgot_username.title')}</strong>
              <Trans i18nKey="new_account_activation.forgot_username.text">
                <p>
...
export default observer(ResendAccountActivationStep1);
